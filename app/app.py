...
planet_df = pd.read_csv("Data/planets_web_ready.csv")


# make csv that corresponds to this
# orbit_df = pd.read_sql_table('orbital_radius', engine)
# make csv that corresponds to this -- maybe merge it before
# it appears taht orbital data in confirmed_planets_with_habitability.csv


# planet_df['Distance from Host Star'] = planet_df['orbit_semi_major_axis']
# z = planet_df['host_name'].value_counts()
# z1 = z.to_dict()
# planet_df['Number of Planets in System'] = planet_df['host_name'].map(z1)
# planet_df
# conditions = [((planet_df['equilibrium_temperature'] >= 200) & (planet_df['equilibrium_temperature'] <= 320)
